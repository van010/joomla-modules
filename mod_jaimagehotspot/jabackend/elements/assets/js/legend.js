...
jQuery(document).ready(function ($) {
	...
	if ($('#attrib-settings').length) {
		if (parseFloat(jversion) >= 4) {
			jClass = ' j4 j5 ';
			$('#fieldset-settings').addClass('ja-panel');
		} else {
			$('#attrib-settings').addClass('ja-panel');
		}
	}
	...
	$('.ja-panel').each(function () {
		var $pane = $(this);
		var $topLegends = $pane.find('.top-legend-group');
		const fieldSetting = $pane.find('#fieldset-settings');
