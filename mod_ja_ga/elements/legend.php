	...
	public function setup(SimpleXMLElement $element, $value, $group = null)
	{
		...
		if (version_compare(JVERSION, '4.0', 'lt')){
			...
			jQuery(document).ready(function($){
				var j4Tablist = jQuery(\"joomla-tab#myTab > div[role='tablist']\");
				var j3Tablist = jQuery('ul#myTabTabs');
				console.log(j4Tablist);
				
				if (j4Tablist.length){
					j4Tablist.children('button').each(function (idx, el) {
					var el = $(el);
					if (el.attr('aria-controls') === 'attrib-Options'){
						el.after($('button[aria-controls=\"assignment\"]'));
					}
				})}})
			");
		}
